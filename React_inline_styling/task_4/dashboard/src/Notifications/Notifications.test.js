...
describe('Notifications', () => {
  ...
  describe('when displayDrawer is true', () => {
    ...
    it('still renders the menu item', () => {
      expect(wrapper.find('div').first().text()).toBe('Your notifications');
    });

    it('hides the menu item now that the panel is open', () => {
      const closedClass = shallow(<Notifications displayDrawer={false} />)
        .find('div')
        .first()
        .prop('className');
      const openClass = wrapper.find('div').first().prop('className');

      expect(openClass).not.toBe(closedClass);
    });
